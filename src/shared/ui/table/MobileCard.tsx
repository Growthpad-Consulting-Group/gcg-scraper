...
const MobileCard: React.FC<MobileCardProps> = ({
  ...
  return (
    <div
      key={row.id || index}
      className={`px-5 py-5 border-b border-slate-200/60 dark:border-slate-700/40 transition-all duration-200 ${index % 2 === 0
          ...
          {(() => {
            ...
            return (
              <div className="relative" ref={dropdownRef}>
                ...
                />
                {showDropdown && (
                  <div className="absolute right-0 top-10 z-[100] min-w-[180px] rounded-2xl shadow-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1.5 overflow-hidden">
                    {visibleActions.map((action, i) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isDisabled}
                          className={`flex items-center gap-3 w-full px-4 py-3 text-sm font-medium rounded-xl transition-all ${isDisabled ? "opacity-40" : "hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300"
                            }`}
                        >
