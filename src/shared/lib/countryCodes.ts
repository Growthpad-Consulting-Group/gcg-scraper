// ISO 3166-1 alpha-2 codes for every country in the tenders feature's WORLD_COUNTRIES list —
// lets a free-text location ("Nairobi, Kenya") be turned into a real geographic constraint
// (Apify's Google Maps actor accepts a `countryCode` input) instead of relying purely on the
// search string text, which Google Maps can occasionally interpret loosely.
import { WORLD_COUNTRIES } from "@/features/tenders/api/countries";
...
const ISO2_BY_NAME: Record<string, string> = {
  ...
  "Republic of the Congo": "CG", Congo: "CG", "Costa Rica": "CR", "Cote d'Ivoire": "CI",
  ...
  Guatemala: "GT", "Guinea-Bissau": "GW", Guinea: "GN", Guyana: "GY", Haiti: "HT",
  ...
  "Trinidad and Tobago": "TT", Tunisia: "TN", Turkiye: "TR", Turkey: "TR",
...
/** Pulls an ISO 3166-1 alpha-2 code out of free-text location input ("Nairobi, Kenya" -> "KE").
 * Longest-name-first so e.g. "Guinea-Bissau" matches before the substring "Guinea". Returns
 * undefined when no recognizable country is found, rather than guessing — callers should treat
 * that as "no hard geo-filter available", not an error. */
export function extractCountryCode(location: string | null | undefined): string | undefined {
  if (!location) return undefined;
  const trimmed = location.trim();
  if (!trimmed) return undefined;

  for (const name of SORTED_NAMES) {
    if (new RegExp(`\\b${escapeRegExp(name)}\\b`, "i").test(trimmed)) return ISO2_BY_NAME[name];
  }
  return undefined;
}
