...
// unnoticed after its initial notification scrolled off. Window is configurable from the
// Settings page (reminder_window_days) — see shared/lib/appSettings.ts for the default.

export const sendClosingRemindersJob = inngest.createFunction(
  { id: "send-closing-reminders", retries: 0, triggers: { cron: "0 6 * * *" } },
  async ({ step }) => {
    const supabase = createServerSupabaseClient();
    const { reminder_window_days: reminderWindowDays } = await step.run("load-settings", () => getAppSettings(supabase));

    const cutoff = new Date(Date.now() + reminderWindowDays * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    await step.run("send-slack-reminder", async () => {
      ...
      const header = `⏰ ${tenders.length} tender${tenders.length === 1 ? "" : "s"} closing within ${reminderWindowDays} days`;
