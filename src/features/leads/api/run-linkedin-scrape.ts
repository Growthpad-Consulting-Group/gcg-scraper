...
export const runLinkedInScrapeJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      // Same failure-tolerant stance as a bad/empty Apify run above: an exception before or
      // during the run (bad input, network error) degrades to "done, 0 leads" rather than
      // leaving the job stuck at "running" forever (confirmed live as a real failure mode on the
      // sibling GMB job — see run-gmb-scrape.ts).
      await step.run("mark-done-exception", async () => {
        await supabase
          .from("scrape_jobs")
          .update({ status: "done", finished_at: new Date().toISOString(), result_summary: { leadsFound: 0, error: err?.message ?? "Unknown error" } })
          .eq("id", jobId)
          .neq("status", "canceled");
      });
      return { jobId, leadsFound: 0, error: err?.message };
    }
  }
);
